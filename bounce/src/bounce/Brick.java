...
public class Brick extends Entity {
	...
	public void configBricks(StateBasedGame game, int levels) {
		...
		switch (levels) {
		case 1:
			// System.out.println("configBricks, bg.bricks.size() =" +
			// bg.bricks.size());
			for (int i = 0; i < 17; i ++) {
				for (int j = 0; j < 3; j++) {
	...
	public void changePic(int levels) {
		...
		case 1:
			...
			break;
		case 2:
			removeImage(ResourceManager.getImage(BounceGame.BRICK_RSC));
	...
	public void update(int levels) {
		...
		case 2: // for first hit, just change the image to skull
			...
			removeImage(ResourceManager.getImage(BounceGame.ZOMBIE_RSC));
			addImageWithBoundingBox(ResourceManager
					.getImage(BounceGame.SKULL_RSC)); // for second hit, the
														// bricks were destroyed

			break;
		case 3:
			removeImage(ResourceManager.getImage(BounceGame.COIN_RSC));
			addImageWithBoundingBox(ResourceManager
					.getImage(BounceGame.SKULL_RSC)); // for second hit, the
														// bricks were destroyed
		default:
